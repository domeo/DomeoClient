...
import org.mindinformatics.gwt.domeo.plugins.persistence.annotopia.ui.existingsets.AnnotationSetTreeViewModel.AnnotationSetInfo;
import org.mindinformatics.gwt.domeo.plugins.persistence.annotopia.ui.existingsets.AnnotationSetTreeViewModel.Category;
...
//	  CellTree cellTree;
	...
	// ------------------------------------------------------------------------
	public ExistingAnnotationViewerPanel(IDomeo domeo, List<MAnnotopiaAnnotationSet> annotationSets) {
		_domeo = domeo;
		...
		try {		
			...
			for(int i=0; i< annotationSets.size(); i++) {
				if(_domeo.getAnnotationPersistenceManager().isAnnotationSetLoaded(annotationSets.get(i).getId()))
					continue;
...
//					publicList.add(new AnnotationSetInfo(categories[0], (JsoAnnotationSetSummary)responseOnSets.get(i)));
					addAnnotationSet(new AnnotationSetInfo(categories[0], annotationSets.get(i)));
