 ...
 */
public class StatementsResultsListPanel extends ResizeComposite {
	...
	public interface Listener {
		void onItemSelected(MMicroPublicationAnnotation term);
	}

	interface Binder extends UiBinder<Widget, StatementsResultsListPanel> {
	}
	...
	ArrayList<MMicroPublicationAnnotation> _references;
	HashMap<String, MMicroPublicationAnnotation> _currentReferences;
	ArrayList<ReferenceCheckBox> _checkBoxes;

	private IDomeo _domeo;
	private StatementsSearchWidget _parentWidget;
	private WidgetUtils _widgetsUtils;
	private boolean _singleSelection;

	public StatementsResultsListPanel(IDomeo domeo, StatementsSearchWidget parentWidget,
			ArrayList<MMicroPublicationAnnotation> references,
			HashMap<String, MMicroPublicationAnnotation> currentReferences,
	...
	public void selectRow(int row) {
		...
		MMicroPublicationAnnotation item = _references.get(startIndex + row);
		if (item == null) {
			return;
		}
	...
	public void refreshAttachedBibliographicObjects() {
		...
		ArrayList<MMicroPublicationAnnotation> bos = _parentWidget
				.getMicroPublicationObjects();
		for (MMicroPublicationAnnotation bo : bos) {
			_currentReferences.put(bo.getIndividualUri(), bo);
		}
	}
	...
	public void update() {
		...
		try {
			for (int i = 0; i < _references.size(); i++) {
				MMicroPublicationAnnotation publication = _references.get(i);
				...
				*/
				...
				table.setWidget(i, 1, new HTML(publication.getMicroPublication().getArgues().getText()));
	...
	public void update(String sourceIdFilter) {
		...
		try {
			...
			for (int i = 0; i < _references.size(); i++) {
				...
				/*
				if (!StatementsSearchBar.ALL_URI.equals(sourceIdFilter)
	...
	private void checkSelection() {
		ArrayList<MMicroPublicationAnnotation> publications = getSelectedPublications();

		for (MMicroPublicationAnnotation _publication : publications) {
			_parentWidget.addStatement(_publication);
	...
	public ArrayList<MMicroPublicationAnnotation> getSelectedPublications() {
		ArrayList<MMicroPublicationAnnotation> publications = new ArrayList<MMicroPublicationAnnotation>();
	...
	public ArrayList<MMicroPublicationAnnotation> getReferences() {
		return _references;
	}
...
class ReferenceCheckBox extends CheckBox {
	...
	private StatementsResultsListPanel _parentWidget;
	private MMicroPublicationAnnotation _publication;
	
	private StatementsSearchWidget _searchWidget;

	public ReferenceCheckBox(StatementsSearchWidget searchWidget,
			StatementsResultsListPanel parentWidget,
			MMicroPublicationAnnotation publication, int row) {
		super();
		_this = this;
		...
		this.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				if (_this.getValue()) {
					...
					_searchWidget.addStatement(_publication);
	...
	public MMicroPublicationAnnotation getPublication() {
		return _publication;
	}
}