 ...
 * under the License.
 */
package org.mindinformatics.gwt.domeo.plugins.annotation.commentaries.linear.ui.tile;
 ...
 */
public class TLinearCommentTile extends ATileComponent implements ITileComponent {

	interface Binder extends UiBinder<Widget, TLinearCommentTile> { }
	private static final Binder binder = GWT.create(Binder.class);
	
	// By contract 
	private MLinearCommentAnnotation _annotation;
	...
	public TLinearCommentTile(IDomeo domeo, IAnnotationEditListener listener) {
	...
	public void initializeLens(MAnnotation annotation) {
		_annotation = (MLinearCommentAnnotation) annotation;
		refresh();
	}	
	@Override
			...
			submitButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					MLinearCommentAnnotation annotation = LinearCommentsFactory.createLinearComment(
							...
							_domeo.getAgentManager().getSoftware(), selector, addCommentBody.getText());
					...
					if(((BooleanPreference)_domeo.getPreferences().getPreferenceItem(Domeo.class.getName(), Domeo.PREF_DISPLAY_USER_PROVENANCE)).getValue()) {
						...
						if(SelectorUtils.isOnMultipleTargets(annotation.getSelectors())) { 
							...
							if(SelectorUtils.isOnAnnotation(annotation.getSelectors())) {
								if(((MLinearCommentAnnotation)annotation).getTitle()!=null && ((MCommentAnnotation)annotation).getTitle().trim().length()>0) {
