 ...
 */
public class ExistingAnnotationViewerPanel2 extends Composite implements
		IContainerPanel, IContentPanel, IResizable {

	private static final String TITLE = "Import of Existing Annotation Sets";

	interface Binder extends
			UiBinder<HorizontalPanel, ExistingAnnotationViewerPanel2> {
	}
	...
	public ExistingAnnotationViewerPanel2(IDomeo domeo, JsArray responseOnSets) {
		...
		int top = ((!_domeo.getProfileManager().getUserCurrentProfile()
				.isFeatureDisabled(IProfile.FEATURE_PUBLIC_ANNOTATION)) ? 1 : 0)
				+ ((!_domeo.getProfileManager().getUserCurrentProfile()
						.isFeatureDisabled(IProfile.FEATURE_GROUP_ANNOTATION)) ? 1
						: 0)
				+ ((!_domeo.getProfileManager().getUserCurrentProfile()
						.isFeatureDisabled(IProfile.FEATURE_PRIVATE_ANNOTATION)) ? 1
						: 0)
				+ ((!_domeo
						.getProfileManager()
						.getUserCurrentProfile()
						.isFeatureDisabled(IProfile.FEATURE_EXTERNAL_ANNOTATION)) ? 1
						: 0);
		...
		if (!_domeo.getProfileManager().getUserCurrentProfile()
				.isFeatureDisabled(IProfile.FEATURE_PUBLIC_ANNOTATION))
			categories[counter++] = new Category(catNames[0],
					images.publicAccess_24_bn());

		if (!_domeo.getProfileManager().getUserCurrentProfile()
				.isFeatureDisabled(IProfile.FEATURE_GROUP_ANNOTATION))
			categories[counter++] = new Category(catNames[1],
					images.groupsAccess_24());

		if (!_domeo.getProfileManager().getUserCurrentProfile()
				.isFeatureDisabled(IProfile.FEATURE_PRIVATE_ANNOTATION))
			categories[counter++] = new Category(catNames[2],
					images.privateAccess());

		if (!_domeo.getProfileManager().getUserCurrentProfile()
				.isFeatureDisabled(IProfile.FEATURE_EXTERNAL_ANNOTATION))
			categories[counter++] = new Category(catNames[3],
					images.boxIcon_24());
		...
		try {
			...
			for (int i = 0; i < responseOnSets.length(); i++) {
				if (_domeo.getAnnotationPersistenceManager()
						.isAnnotationSetLoaded(
								((JsoAnnotationSetSummary) responseOnSets
										.get(i)).getId()))
					continue;

				if (!_domeo.getProfileManager().getUserCurrentProfile()
						.isFeatureDisabled(IProfile.FEATURE_PUBLIC_ANNOTATION)
						&& ((JsoAnnotationSetSummary) responseOnSets.get(i))
								.isPublic()) {
					publicList.add(new AnnotationSetInfo(categories[0],
							(JsoAnnotationSetSummary) responseOnSets.get(i)));
					addAnnotationSet(new AnnotationSetInfo(categories[0],
							(JsoAnnotationSetSummary) responseOnSets.get(i)));
					categories[0].incrementCounter();
				} else if (!_domeo.getProfileManager().getUserCurrentProfile()
						.isFeatureDisabled(IProfile.FEATURE_GROUP_ANNOTATION)
						&& ((JsoAnnotationSetSummary) responseOnSets.get(i))
								.isGroups()) {
					groupsList.add(new AnnotationSetInfo(categories[1],
							(JsoAnnotationSetSummary) responseOnSets.get(i)));
					addAnnotationSet(new AnnotationSetInfo(categories[1],
							(JsoAnnotationSetSummary) responseOnSets.get(i)));
					categories[1].incrementCounter();
				} else if (!_domeo.getProfileManager().getUserCurrentProfile()
						.isFeatureDisabled(IProfile.FEATURE_PRIVATE_ANNOTATION)
						&& ((JsoAnnotationSetSummary) responseOnSets.get(i))
								.getPermissionsAccessType().equals(
										_domeo.getAgentManager()
												.getUserPerson().getUri())) {
					privateList.add(new AnnotationSetInfo(categories[2],
							(JsoAnnotationSetSummary) responseOnSets.get(i)));
					addAnnotationSet(new AnnotationSetInfo(categories[2],
							(JsoAnnotationSetSummary) responseOnSets.get(i)));
			...
			final MultiSelectionModel<AnnotationSetInfo> selectionModel = new MultiSelectionModel<AnnotationSetInfo>(
					AnnotationSetTreeViewModel.AnnotationSetInfo.KEY_PROVIDER);
			selectionModel
					.addSelectionChangeHandler(new SelectionChangeEvent.Handler() {
						public void onSelectionChange(SelectionChangeEvent event) {
							...
							importButton.setText("Import " + selected.size()
			...
			_domeo.getLogger().debug(this,
					"ExistingAnnotationViewerPanel " + e.getMessage());
		}
