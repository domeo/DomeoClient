 ...
 */
public class TCurationTile extends ATileComponent implements ITileComponent {

	interface Binder extends UiBinder<Widget, TCurationTile> { }
	private static final Binder binder = GWT.create(Binder.class);
	
	// By contract 
	private MCurationToken _annotation;
	...
	public TCurationTile(IDomeo domeo, IAnnotationEditListener listener) {
	...
	public void initializeLens(MAnnotation annotation) {
		_annotation = (MCurationToken) annotation;
		refresh();
	}	
	@Override
		...
		try {
			...
			submitButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					TCurationTile c = new TCurationTile(_domeo, _listener);
					...
					if(((BooleanPreference)_domeo.getPreferences().getPreferenceItem(Domeo.class.getName(), Domeo.PREF_DISPLAY_USER_PROVENANCE)).getValue()) {
						...
						if(SelectorUtils.isOnMultipleTargets(annotation.getSelectors())) { 
							...
							if(SelectorUtils.isOnAnnotation(annotation.getSelectors())) {
								if(_annotation.getStatus().equals(MCurationToken.CORRECT_BROAD)) {
									Image ic = new Image(Domeo.resources.acceptBroadIcon());
									ic.setTitle("Curation token");
									provenance.add(ic);
									provenance.setCellWidth(ic, "18px");
								} else if(_annotation.getStatus().equals(MCurationToken.CORRECT)) {
									Image ic = new Image(Domeo.resources.acceptIcon());
									ic.setTitle("Curation token");
									provenance.add(ic);
									provenance.setCellWidth(ic, "18px");
								} else if(_annotation.getStatus().equals(MCurationToken.INCORRECT)) {
									Image ic = new Image(Domeo.resources.rejectIcon());
									ic.setTitle("Curation token");
									provenance.add(ic);
									provenance.setCellWidth(ic, "18px");
								} 
