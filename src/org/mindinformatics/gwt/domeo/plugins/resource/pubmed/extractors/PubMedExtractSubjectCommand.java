...
public class PubMedExtractSubjectCommand extends APubMedBibliograhyExtractorCommand implements ICommand, 
		IPubMedItemsRequestCompleted {

	private IDomeo _domeo;
	private ICommandCompleted _completionCallback;
	
	public PubMedExtractSubjectCommand(IDomeo domeo, ICommandCompleted completionCallback) {
	...
	public void execute() {
		if(_domeo.getAnnotationPersistenceManager().getBibliographicSet().getLevel()>0) {
			...
			try {
				...
				List<Element> elements = HtmlUtils.getElementsByClassName(frameDocument, "rprtid", true);
				if(elements.size()==1) {
					Element e = elements.get(0);
					if(e.getChildCount()>2) {
						String pubmedId = e.getInnerText().substring(e.getInnerText().indexOf(":")+1, e.getInnerText().indexOf("[")-1).trim();
						if(pubmedId!=null && pubmedId.trim().length()>0) {
							...
							try {
								...
								pubMedManager.getBibliographicObject(this, PUBMED_ID, pubmedId);
							} catch(Exception exc) {
