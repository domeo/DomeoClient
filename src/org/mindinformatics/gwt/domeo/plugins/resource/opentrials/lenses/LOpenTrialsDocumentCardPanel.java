package org.mindinformatics.gwt.domeo.plugins.resource.opentrials.lenses;
 ...
 */
public class LOpenTrialsDocumentCardPanel extends Composite implements IRefreshableComponent, IResourceLensComponent {
	
	public static final String KNOWN_RESOURCE = "KNOWN RESOURCE";
	
	interface Binder extends UiBinder<Widget, LOpenTrialsDocumentCardPanel> { }
	...
	private MOpenTrialsDocument _resource;
	...
	public void initializeLens(MGenericResource resource, HashMap<String, String> parameters) {
		...
		try {
			...
			body.clear();
			body.add(new HTML("<img src='" + _resources.crossLittleIcon().getSafeUri().asString() + "'/> Exception while initialing OpenTrials resource info " + e.getMessage()));
		}
	}
	...
	public void refresh() {
		...
		try {
			...
			_domeo.getLogger().exception(this, "Exception while rendering OpenTrials resource info " + e.getMessage());
			body.clear();
			body.add(new HTML("<img src='" + _resources.crossLittleIcon().getSafeUri().asString() + "'/> Exception while rendering OpenTrials resource info " + e.getMessage()));
		}
