...
public abstract class AFormComponent extends Composite implements
		IFormComponent, IResizable {
	...
	protected void refreshAnnotationSetFilter(ListBox annotationSet,
			MAnnotation _item) {
		annotationSet.clear();
		...
		// This line allows Domeo to create a new 'Default set' if no sets have
		// been
		// created yet
		MAnnotationSet currentSet = _domeo.getAnnotationPersistenceManager()
				.getCurrentSet();
		ArrayList<MAnnotationSet> sets = _domeo
				.getAnnotationPersistenceManager().getAllUserSets();
		...
		for (MAnnotationSet set : sets) {
			...
			if (_item == null) {
				...
				if (set.getLocalId().equals(
						_domeo.getAnnotationPersistenceManager()
								.getSetByAnnotationId(_item.getLocalId())
								.getLocalId())) {
					selection = counter;
				}
			}

			// System.out.println("set label: " + set.getLabel());

			if (!set.getLabel().equals("NLP Drug Mention Set")) {
				if (set.getCreatedBy()
						.getUri()
		...
		// System.out.println("annotationSet size: " +
		// annotationSet.getItemCount());
	...
	protected void displayDialog(String message, boolean close) {
		Window.alert(message
				+ " - If the problem persist it should be reported!");
		if (close)
