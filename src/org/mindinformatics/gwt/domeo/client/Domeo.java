 ...
 */
public class Domeo extends Application implements IDomeo, EntryPoint, /*
																	 * IRetrieveExistingBibliographySetHandler
																	 * ,
																	 */
IRetrieveExistingAnnotationSetHandler,
		IRetrieveExistingAnnotationSetListHandler {
	...
	public void onModuleLoad() {
		_domeo = this;
		this.logger.info(this, "Creating Domeo (" + Domeo.APP_VERSION + ") "
				+ getStartingMode());
		_this = this;
	...
	public void completeInitialization() {
		...
		if (_persistenceManager != null) {
			this.getLogger().debug(
					...
					"Persistence manager selected "
							+ _persistenceManager.getClass().getName());
		...
		resourcePanelsManager.registerResourcePanel(
				MGenericResource.class.getName(),
		...
		resourcePanelsManager.registerResourcePanel(
				MDocumentResource.class.getName(),
		...
		if (_profileManager.getUserCurrentProfile().isPluginEnabled(
				OmimPlugin.getInstance().getPluginName())) {
			resourcePanelsManager.registerResourcePanel(MOmimDocument.class
					.getName(), new LOmimDocumentCardPanel(this));
			pluginsManager.enablePlugin(OmimPlugin.getInstance(), true);
		}
		// Open Trials Plugin
		pluginsManager.registerPlugin(OpenTrialsPlugin.getInstance());
		if (_profileManager.getUserCurrentProfile().isPluginEnabled(
				OpenTrialsPlugin.getInstance().getPluginName())) {
			resourcePanelsManager.registerResourcePanel(
					MOpenTrialsDocument.class.getName(),
		...
		if (_profileManager.getUserCurrentProfile().isPluginEnabled(
				PubMedPlugin.getInstance().getPluginName())) {
			resourcePanelsManager.registerResourcePanel(
					MPubMedDocument.class.getName(),
					new LPubMedDocumentCardPanel(this));
			pluginsManager.enablePlugin(PubMedPlugin.getInstance(), true);
			if (_profileManager.getUserCurrentProfile().isPluginEnabled(
					PmcPlugin.getInstance().getPluginName())) {
		...
		annotationCardsManager.registerAnnotationCard(
				MAnnotationCitationReference.class.getName(),
...
/*
		...
		// false);
		if (_profileManager.getUserCurrentProfile().isPluginEnabled(
				MicroPublicationsPlugin.getInstance().getPluginName())) {
			annotationFormsManager.registerAnnotationForm(
		...
		annotationTailsManager.registerAnnotationTile(MCommentAnnotation.class
				.getName(), new CommentTileProvider(this));
		annotationTailsManager.registerAnnotationTile(
				MLinearCommentAnnotation.class.getName(),
		...
		if (((BooleanPreference) getPreferences().getPreferenceItem(
			...
			LinearCommentsSidePanel commentsSidePanel = new LinearCommentsSidePanel(
					this, sidePanelsFacade, discussionSideTab);
			sidePanelsFacade.registerSideComponent(commentsSideTab,
					commentsSidePanel, (LinearCommentsSideTab.HEIGHT + 16)
							+ "px");
		}
		...
		if (((BooleanPreference) getPreferences().getPreferenceItem(
				Domeo.class.getName(), Domeo.PREF_DISPLAY_ANNOTATION_FOR_DEBUG))
				.getValue()) {
			final ASideTab annotationsDebugSideTab = new AnnotationDebugSideTab(
					this, sidePanelsFacade);

			AnnotationDebugSidePanel annotationDebugSidePanel = new AnnotationDebugSidePanel(
					this, sidePanelsFacade, annotationsDebugSideTab);
			componentsManager.addComponent(annotationDebugSidePanel);

			sidePanelsFacade.registerSideComponent(annotationsDebugSideTab,
		...
		// ===================================
		TextMiningRegistry.getInstance(this).registerTextMiningService(
				BioPortalManager.getInstance());
		TextMiningRegistry.getInstance(this).registerTextMiningService(
				NifManager.getInstance());
		...
		if (!ApplicationUtils.getUrlParameter("url").isEmpty()) {
			domeoToolbarPanel.getAddressBarPanel().setAddress(
					ApplicationUtils.decodeURIComponent(ApplicationUtils
							.getUrlParameter("url")));
			this.attemptContentLoading(ApplicationUtils
					.decodeURIComponent(ApplicationUtils.getUrlParameter("url")));
		}
	...
	public IUserManager selectUserManager(ICommandCompleted completionCallback) {
		if (this.getUserManager() == null) {
			if (isStandaloneMode()) {
				this.setUserManager(new StandaloneUserManager(
						(IApplication) this, completionCallback));
			} else {
				if (isHostedMode()) {
					if (isJsonFormat()) {
						this.setUserManager(new JsonUserManager(
								(IApplication) this, completionCallback));
					} else {
						this.setUserManager(new GwtUserManager(
								(IApplication) this, completionCallback));
					}
				} else {
					this.setUserManager(new JsonUserManager(
					...
					// throw new
					// RuntimeException("UserManager not implemented for a real situation");
				}
			}
		}
	...
	public IAgentManager selectAgentManager(ICommandCompleted completionCallback) {
		if (this.getAgentManager() == null) {
			if (isStandaloneMode()) {
				this.setAgentManager(new StandaloneAgentManager(
						(IApplication) this, completionCallback));
			} else {
				if (isHostedMode()) {
					if (isJsonFormat()) {
						this.setAgentManager(new JsonAgentManager(
								(IApplication) this, completionCallback));
					} else {
						this.setAgentManager(new GwtAgentManager(
								(IApplication) this, completionCallback));
					}
				} else {
					// Real service
					...
					// throw new
					// RuntimeException("AgentManager not implemented for a real situation");
				}
			}
		}
	...
	/* Should provide the right user manager */
	public IProfileManager selectProfileManager(
			ICommandCompleted completionCallback) {
		if (this.getProfileManager() == null) {
			if (isStandaloneMode()) {
				this.setProfileManager(new StandaloneProfileManager(
						(IApplication) this, completionCallback));
			} else {
				if (isHostedMode()) {
					if (isJsonFormat()) {
						this.setProfileManager(new JsonProfileManager(
								(IApplication) this, completionCallback));
					} else {
						this.setProfileManager(new GwtProfileManager(
								(IApplication) this, completionCallback));
					}
				} else {
					// Real service
					...
					// throw new
					// RuntimeException("ProfileManager not implemented for a real situation");
				}
			}
		}
	...
	public IReportsManager selectReportManager(
			ICommandCompleted completionCallback) {
		if (this.getReportManager() == null) {
			if (isStandaloneMode()) {
				this.setReportManager(new StandaloneReportManager(
						(IApplication) this, completionCallback));
			} else {
				if (isHostedMode()) {
					if (isJsonFormat()) {
						this.setReportManager(new JsonReportManager(
								(IApplication) this, completionCallback));
					} else {
						this.setReportManager(new GwtReportManager(
								(IApplication) this, completionCallback));
					}
				} else {
					// Real service
					...
					// throw new
					// RuntimeException("ReportsManager not implemented for a real situation");
				}
			}
		}
	...
	public IPersistenceManager selectPersistenceManager() {
		if (isStandaloneMode()) {
			...
			if (isHostedMode()) {
				...
				return new JsonPersistenceManager(this, null);
				// throw new
				// RuntimeException("PersistenceManager not implemented for a real situation");
			}
		}
	}
	...
	public void resetContentLoading() {
		domeoToolbarPanel.getAddressBarPanel().setAddress(
				_persistenceManager.getCurrentResourceUrl());
	...
	public void attemptContentLoading(String url) {
		if (!contentLoading) {
			contentLoading = true;
			this.getLogger().command(this.getClass().getName(),
					"Attempt loading of " + url);
			...
			if (!_persistenceManager.isResourceLoaded()) {
				...
				LoadDocumentQuestionDialog lwp = new LoadDocumentQuestionDialog(
						this,
						"Do you want to load the new page in the current tab?",
						url);
				addDialogGlassPanel(lwp);
			} else {
				// Reload document
				ReloadDocumentQuestionDialog lwp = new ReloadDocumentQuestionDialog(
						this,
						"Do you want to reload the page in the current tab?",
						url);
				addDialogGlassPanel(lwp);
			}
		}
	...
	public void loadContent(String url) {
		LoadingDocumentMessageDialog ldmd = new LoadingDocumentMessageDialog(
				this, "Loading...", url);
		...
		if (!GWT.isScript() || url.indexOf("tests/") >= 0) {
			if (url.endsWith("tests/OMIM253300.html")) {
				...
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://www.ncbi.nlm.nih.gov/pmc/articles/PMC2759694/");
			} else if (url.endsWith("tests/PMC3639628.html")) {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3639628/");
			} else if (url.endsWith("tests/PMC2759694_v062012.html")) {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://www.ncbi.nlm.nih.gov/pmc/articles/PMC2759694/");
			} else if (url.endsWith("tests/PM17561409.html")) {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://www.ncbi.nlm.nih.gov/pubmed/17561409");
			} else if (url.endsWith("tests/PMC2700002.html")) {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://www.ncbi.nlm.nih.gov/pmc/articles/PMC2700002/");
				...
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://www.foaffy.org/gallery2/main.php");
			} else if (url.endsWith("tests/NCT000001549.html")) {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://clinicaltrials.gov/ct2/show/NCT000001549");
			} else if (url.endsWith("tests/NCT00368199.html")) {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://clinicaltrials.gov/ct2/show/NCT00368199");
			} else if (url.endsWith("tests/NCT01136213.html")) {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://clinicaltrials.gov/ct2/show/NCT01136213");
			} else if (url.endsWith("tests/PMC3308009_v082012.html")) {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url,
						"http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3308009/");
			} else if (url.indexOf("SPL-annotation") >= 0) {
				getContentPanel()
						.getAnnotationFrameWrapper()
						.setUrl(url,
								url.replace("tests/SPL-annotation/",
										"http://dbmi-icode-01.dbmi.pitt.edu:2020/SPL-annotation/"));
			} else {
				getContentPanel().getAnnotationFrameWrapper().setUrl(url);
			}
		} else {
			...
			if (isLocalResources()) {
				...
				getLogger().info(this,
						"Attempting to open a remote resoruce " + url);

				// Filters: can modify the requested URL to reach an appropriate
				...
				proxyProtocol = (proxyProtocol.trim().equals("http") || proxyProtocol
						.trim().equals("https")) ? proxyProtocol : "http";
				if (!url.endsWith(".pdf")) {
					String PROXY = proxyProtocol
							+ "://"
							+ ApplicationUtils.getHostname(GWT
									.getHostPageBaseURL()) + "/proxy/";
					getContentPanel().getAnnotationFrameWrapper().setUrl(
							PROXY + proxyUrl, url);
					getLogger().info(this, "Proxy call " + PROXY + proxyUrl);
				} else {
					String PREFIX = ApplicationUtils
							.getUrlBase(ApplicationUtils.getUrlString())
							+ "web/pdf?pdf=";
					String PROXY = proxyProtocol
							+ "://"
							+ ApplicationUtils.getHostname(GWT
	...
	/**
	 * This is used to calculate the duration of the different stages of the
	 * pipeline triggered by the document loaded completion
	 */
	...
	public static void notifyDocumentLoadedStageOneStatic() {
		if (_domeo != null)
			_domeo.notifyDocumentLoadedStageOneAfter();
	}

	// For PDF
	@Override
	public void notifyDocumentLoadedStageOne() {
		if (!getContentPanel().getAnnotationFrameWrapper().getUrl()
				.endsWith(".pdf"))
			notifyDocumentLoadedStageOneAfter();
	}

	private void notifyDocumentLoadedStageOneAfter() { // Document loaded in the
														// browser

		try {
			documentPipelineTimer = System.currentTimeMillis();
			this.getLogger().debug(this, "AFTER DOCUMENT",
					"Executing notifyDocumentLoadedStageOne()");

			contentLoading = false;
			domeoToolbarPanel
					.attachGroup(DomeoToolbarPanel.DOCUMENT_COMMANDS_GROUP);

			// Enables the right side panels
			getSidePanelsFacade().setActive(true);

			// Resource metadata extraction
			((ProgressMessagePanel) ((DialogGlassPanel) _dialogPanel)
					.getPanel()).setMessage("Resource Metadata extractor");
			...
			resource.setLabel(contentPanel.getAnnotationFrameWrapper()
					.getDocumentTitle());

			// Persistence
			((ProgressMessagePanel) ((DialogGlassPanel) _dialogPanel)
					.getPanel()).setMessage("Persistence manager");
			this.getLogger().debug(this.getClass().getName(),
					"Persistence manager setup");
			_persistenceManager.setResourceLoaded(true);
			_persistenceManager.setCurrentResource(resource); // Overridden by a
																// more
																// appropriate
																// one
			...
			this.getLogger().exception(this,
					"::notifyDocumentLoaded " + e.getMessage());
		}
	}
	...
	public void notifyNotProxiedDocumentLoaded() {
		this.getLogger().warn(
				...
				"Document loaded "
						+ contentPanel.getAnnotationFrameWrapper().getUrl());
		this.getLogger()
				.warn(this.getClass().getName(),
						"Impossible to access the document content, did you forget to go through the proxy?");
		_dialogPanel.hide();
	}
	...
	public void notifyDocumentLoadedStageTwo() {
		...
		this.getLogger().debug(
				this,
				"AFTER DOCUMENT",
				"Completed Execution of notifyDocumentLoadedStageOne() in "
						+ (System.currentTimeMillis() - documentPipelineTimer)
						+ "ms");

		// documentPipelineTimer = System.currentTimeMillis();
		// this.getLogger().debug(this, "AFTER DOCUMENT",
		// "Executing startBibliographyRetrieval()");
		//
	...
	// public void setExistingBibliographySetList(JsArray responseOnSets,
	// boolean isVirtual) {
	...
	//
	// this.getLogger().debug(this, "AFTER DOCUMENT",
	// "Completed Execution of startBibliographyRetrieval() in " +
	// (System.currentTimeMillis()-documentPipelineTimer)+ "ms");
	//
	// documentPipelineTimer = System.currentTimeMillis();
	// this.getLogger().debug(this, "AFTER DOCUMENT",
	// "Executing startExtraction()");
	...
	public void startExtraction() {
		// Extractor
		((ProgressMessagePanel) ((DialogGlassPanel) _dialogPanel).getPanel())
				.setMessage("Starting extraction pipeline");
		extractorsManager.setUpExtractor(getContentPanel()
				.getAnnotationFrameWrapper().getUrl());
	...
	public void endExtraction() {
		...
		IFrameElement iframe = IFrameElement.as(contentPanel
				.getAnnotationFrameWrapper().getFrame().getElement());
		final Document frameDocument = iframe.getContentDocument();
		// Injecting the stylesheet in the loaded document
		if (frameDocument != null)
			HtmlUtils.addCssStylesheet(frameDocument, getCssManager()
					.createStylesheet());
		...
		Object w = this.getResourcePanelsManager().getResourcePanel(
				((IDomeo) this).getPersistenceManager().getCurrentResource()
						.getClass().getName());
		...
		this.getLogger().debug(
				this,
				"AFTER DOCUMENT",
				"Completed Execution of endExtraction() in "
						+ (System.currentTimeMillis() - documentPipelineTimer)
						+ "ms");
		this.getLogger().debug(this, "AFTER DOCUMENT",
				"Document url: " + ApplicationUtils.getUrlParameter("url"));
		this.getLogger().debug(
				this,
				"AFTER DOCUMENT",
				"Annotation set id: "
						+ ApplicationUtils.getUrlParameter("setId"));

		if (!ApplicationUtils.getUrlParameter("url").isEmpty()
				&& ApplicationUtils.getUrlParameter("url").trim().length() > 13
				&& !ApplicationUtils.getUrlParameter("lineage").isEmpty()) {
			Window.alert("Attempting retrieving existing annotation with lineageId "
					+ ApplicationUtils.getUrlParameter("lineage"));
			Window.alert("Feature not yet implemented");
			((ProgressMessagePanel) ((DialogGlassPanel) _dialogPanel)
					.getPanel())
					.setMessage("Attempting retrieving existing annotation");
			_dialogPanel.hide();
		} else if (!ApplicationUtils.getUrlParameter("url").isEmpty()
				&& ApplicationUtils.getUrlParameter("url").trim().length() > 13
				&& !ApplicationUtils.getUrlParameter("setId").isEmpty()) {
			List<String> uuids = new ArrayList<String>();
			uuids.add(ApplicationUtils.decodeURIComponent(ApplicationUtils
					.getUrlParameter("setId")));
			((DialogGlassPanel) _dialogPanel).hide();
			this.getProgressPanelContainer().setProgressMessage(
					"Retrieving requested annotation");
			((DialogGlassPanel) _dialogPanel).hideSoon();
			this.getAnnotationPersistenceManager()
					.retrieveExistingAnnotationSets(uuids,
							(IRetrieveExistingAnnotationSetHandler) this);
			if (!isLocalResources() && !isHostedMode())
				ApplicationUtils.updateUrl(ApplicationUtils
						.encodeUrlComponent(ApplicationUtils
								.getUrlParameter("url")));
		} else if (!ApplicationUtils.getUrlParameter("url").isEmpty()
				&& ApplicationUtils.getUrlParameter("url").trim().length() > 13
				&& !ApplicationUtils.getUrlParameter("setIds").isEmpty()) {
			List<String> uuids = new ArrayList<String>();
			String[] st = ApplicationUtils.decodeURIComponent(
					ApplicationUtils.getUrlParameter("setIds")).split(",");
			for (int i = 0; i < st.length; i++) {
				uuids.add(st[i]);
				this.getLogger().debug(this,
						"Queuing annotation set with id: " + st[i]);
			}
			...
			if (!isLocalResources() && !isHostedMode())
				ApplicationUtils.updateUrl(ApplicationUtils
						.encodeUrlComponent(this.getPersistenceManager()
		 ...
		 * if(ApplicationUtils.getDocumentUrl()!=null &&
		 * ApplicationUtils.getDocumentUrl().trim().length()>13 &&
		 * ApplicationUtils.getLineageId()!=null &&
		 * ApplicationUtils.getLineageId().trim().length()>0) {
		 * Window.alert("Attempting retrieving existing annotation with lineageId "
		 * + ApplicationUtils.getLineageId());
		 * ((ProgressMessagePanel)((DialogGlassPanel
		 * )_dialogPanel).getPanel()).setMessage
		 * ("Attempting retrieving existing annotation"); _dialogPanel.hide(); }
		 * else if (ApplicationUtils.getDocumentUrl()!=null &&
		 * ApplicationUtils.getDocumentUrl().trim().length()>13 &&
		 * ApplicationUtils.getAnnotationId()!=null &&
		 * ApplicationUtils.getAnnotationId().trim().length()>0) { List<String>
		 * uuids = new ArrayList<String>();
		 * uuids.add(ApplicationUtils.getAnnotationId());
		 * ((DialogGlassPanel)_dialogPanel).hide();
		 * this.getProgressPanelContainer
		 * ().setProgressMessage("Retrieving requested annotation");
		 * 
		 * //((ProgressMessagePanel)((DialogGlassPanel)_dialogPanel).getPanel()).
		 * setMessage("Retrieving requested annotation");
		 * ((DialogGlassPanel)_dialogPanel).hideSoon();
		 * this.getAnnotationPersistenceManager
		 * ().retrieveExistingAnnotationSets(uuids,
		 * (IRetrieveExistingAnnotationSetHandler)this); //_dialogPanel.hide();
		 * } else { checkForExistingAnnotationSets(); }
		 */
	}
	...
	public void checkForExistingAnnotationSets() {
		...
		// ExistingAnnotationViewerPanel(this);
	...
	public void setExistingAnnotationSetList(JsArray responseOnSets) {
		...
		this.getLogger().debug(
				this,
				"Completed Execution of checkForExistingAnnotationSets() in "
						+ (System.currentTimeMillis() - documentPipelineTimer)
						+ "ms");

		if (responseOnSets == null || responseOnSets.length() == 0) {
			// TODO message no annotation found
			this.getProgressPanelContainer().setCompletionMessage(
					"No annotation exist for this document");
		} else {
			this.getProgressPanelContainer().hide();
			try {

				ExistingAnnotationViewerPanel2 lwp = new ExistingAnnotationViewerPanel2(
						this, responseOnSets);
				new EnhancedGlassPanel(this, lwp, lwp.getTitle(), false, false,
						false);
				// _dialogPanel.hide();
	...
	public void displayAnnotationOfSet(MAnnotationSet set) {
		AnnotationSetViewerSideTab tab = new AnnotationSetViewerSideTab(this,
				sidePanelsFacade, set.getLabel().substring(0, 8), 58);
		final AnnotationForSetSidePanel sidePanel = new AnnotationForSetSidePanel(
				this, sidePanelsFacade, tab, new AnnotationSummaryTable(this,
						this.getContentPanel().getAnnotationFrameWrapper()),
				set);
		sidePanel.refresh();

		componentsManager.addComponent(sidePanel);

		ASideTab selectedTab = sidePanelsFacade.registerSideComponent(tab,
				sidePanel, (58 + 16) + "px");
	...
	public void loadExistingAnnotationSetList(JsArray responseOnSets, int size) {
		if (Domeo.verbose)
			this.getLogger().debug(this,
					"Beginning of Domeo.loadExistingAnnotationSetList()");
		if (responseOnSets.length() != size) {
			this.getProgressPanelContainer().setErrorMessage(
					"Something went terribly wrong while retrieving the annotation."
							+ "Only " + responseOnSets.length()
							+ " sets out of " + size + " have been retrieved.");
		...
		if (Domeo.verbose)
			this.getLogger().debug(this, "Beginning of unmarshalling");
		manager.unmarshall(responseOnSets, size);

		if (Domeo.verbose)
			this.getLogger().debug(this, "Unmarshalling completed");
		// this.getDialogPanel().hide();
		if (this.getProgressPanelContainer() != null)
			this.getProgressPanelContainer().setCompletionMessage(
					"Annotation loaded (#sets: " + size + ")!");
	...
	public final native String getAnnotationType(Object obj) /*-{
		return obj['@type'];
	}-*/;

	public final native String getSelectorType(Object obj) /*-{
		return obj['@type'];
	}-*/;

	public final native String getSelectorTargetUrl(Object obj) /*-{
		return obj['ao:annotatesResource'];
	}-*/;

	public final native String getAnnotationTargetUri(Object obj) /*-{
		return obj['ao:annotation'];
	}-*/;

	public final native JsArray<JsAnnotationTarget> getTargets(Object obj) /*-{
		return obj['ao:context'];
	}-*/;

	public final native JsArray<JsoLinkedDataResource> getSemanticTags(
			Object obj) /*-{
		return obj['oax:hasSemanticTag'];
	}-*/;
	...
	public void jumpToLocation(float percentage) {
		...
		// DOM.getElementPropertyInt(frameDocument.getBody().getOffsetHeight(),
		// "height"));
